...
// which are necessary pieces for issuing standard API calls
	...
	// Create the CBV/SRV descriptor heap
	{
		...
		dhDesc.NodeMask = 0; //We want the first gpu
		dhDesc.NumDescriptors = maxConstantBuffers; // we need as many descriptors as constant buffers
	...
	void* data, unsigned int dataSizeInBytes)
{
	...
	// === Copy data to the upload heap ===
	{
		...
		// Increment the offset and lofop back to the beginning if necessary,
	...
	// Create a CBV for this section of the heap
	{
		...
		cbufDesc.BufferLocation = virtualGPUAddress;
		cbufDesc.SizeInBytes = (UINT)reservationSize;
		// Create the CBV, which is a lightweight operation in D3D12
		Device->CreateConstantBufferView(&cbufDesc, cpuHandle);
